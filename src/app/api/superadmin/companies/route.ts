...
export async function POST(request: Request) {
  try {
    ...
    const docData: Record<string, unknown> = {
      ...
      city: city || '',
      country: country || (Array.isArray(countries) && countries.length > 0 ? countries[0] : 'Colombia'),
      countries: Array.isArray(countries) ? countries : (country ? [country] : []),
      ...
      linkedin: linkedin || '',
      xtwitter: xtwitter || '',
      peopleGroup: Array.isArray(peopleGroup) ? peopleGroup : [],
      otherPeopleGroup: otherPeopleGroup || '',
      friendlyBizz: friendlyBizz === true,
      inclusionDEI: inclusionDEI === true,
      membership: membership === true,
      annualRevenue: Number(annualRevenue) || 0,
      collaboratorsCount: Number(collaboratorsCount) || 0,
